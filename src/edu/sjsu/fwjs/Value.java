...
class ClosureVal implements Value {
    ...
    public Value apply(List<Value> argVals) {
    	// Creates a new local environment, with an outer scope of the environment where the function was created.
        Environment newLocalEnv = new Environment(outerEnv);
        
        for (int i = 0; i < argVals.size(); i++)
        {
        	// Each parameter is bound to its matching argument and added to the new local environment.
        	newLocalEnv.createVar(params.get(i), argVals.get(i));
        }
        
        return body.evaluate(newLocalEnv);
    }
}